 ...
 * @brief Mesh-based high-order spatial evaluator for variable-density ACM equations.
 *
...
namespace DNDS::ACMVariable
{
     ...
     * @tparam gDim Mesh dimension, either 2 or 3. The state remains `[rho,mx,my,mz,p]` in both cases.
     * @note Modifier: Runzhi Ma.
     */
    template <int gDim>
    class ACMEvaluator
    ...
    public:
        ...
        {
            Matrix5 left = Matrix5::Zero();  ///< `d integral(F_face) / d U_left`.
            Matrix5 right = Matrix5::Zero(); ///< `d integral(F_face) / d U_right`.
        };
        ...
        /**
         * @brief Apply an independent 5x5 block-Jacobi inverse.
        ...
        /**
         * @brief Return the exact numerical mass flux from the last flow residual.
         * @param face Process-local face.
         * @param quadrature Face quadrature point.
         * @return Oriented mass flux per unit area, shared with RANS transport.
         * @author Runzhi Ma
         */
        real FaceMassFlux(index face, int quadrature) const
        {
            return _faceMassFlux.at(static_cast<std::size_t>(face)).at(quadrature);
        }

    private:
        std::vector<std::vector<real>> _faceMassFlux; ///< Conservative shared flow/RANS mass flux.
         ...
         * @param u Current distributed ACM cell means.
         * @details The generic CFV limiter is reused; only the five-variable ACM characteristic
         ...
         * @return Limited face state `[rho,mx,my,mz,p]`.
         */
         ...
         */
        Eigen::Matrix<real, 3, 5> ReconstructFaceGradient(
            index iCell,
            index iFace,
            rowsize if2c,
            int iG) const;

 ...
 * @brief Runtime-selectable RANS closures for the variable-density ACM solver.
 *
 * @details The turbulence variables are intentionally kept outside the five-component
 ...
 * separation preserves the existing ACM 5x5 characteristic and implicit operators.
...
{
    /**
     * @brief Product-rule source needed by standard SA with conservative rho*nuTilde storage.
     * @param rho Positive local density.
     * @param mu Positive molecular dynamic viscosity.
     * @param nuTilde Local non-negative SA variable.
     * @param densityGradient Spatial gradient of density, not momentum.
     * @param nuGradient Spatial gradient of nuTilde.
     * @return Conservative source -(mu/rho+nuTilde)/sigma * grad(rho).grad(nuTilde).
     * @note Standard nonconservative SA diffusion is rho*div((nu+nuTilde)*grad(nuTilde))/sigma.
     * Moving rho inside the diffusive divergence without this term changes the model.
     * @author Runzhi Ma
     */
    real SADensityDiffusionCorrection(real rho, real mu, real nuTilde,
                                     const Vector3 &densityGradient, const Vector3 &nuGradient);
     ...
     * @param rho0 Positive local density.

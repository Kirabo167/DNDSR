            ...
            for (rowsize iCellFace = 0; iCellFace < cellFaces.size(); iCellFace++)
            {
                ...
                else
                {
                    ...
                    if (_boundaryHandler->GetTypeFromID(
                        ...
                        Velocity(State(flow[iCell])).dot(outward) >= 0)
...
#    pragma omp parallel for schedule(runtime)
        ...
        int iG) const
    {
        ...
        if (reconstructed(0) < _flowSettings.densityFloor)
        {
            const State mean = flow[iCell];
            const real theta = std::clamp((mean(0) - _flowSettings.densityFloor) /
                                         (mean(0) - reconstructed(0)), real(0), real(1));
            reconstructed = mean + theta * (reconstructed - mean);
        }
        ...
        index iCell) const
    {
        ...
        result.template leftCols<gDim>() =
            (_flowGradient[iCell].template middleCols<3>(1) -
             _flowGradient[iCell].col(0) * Velocity(State((*_flow)[iCell])).transpose()).transpose() /
            (*_flow)[iCell](0);
        return result;
    }
...
#    pragma omp parallel for schedule(runtime)
        ...
        for (index iFace = 0; iFace < _mesh->NumFaceProc(); iFace++)
        {
            ...
            for (int iG = 0; iG < quadrature.GetNumPoints(); iG++)
            {
                ...
                Eigen::Matrix<real, 3, 5> flowGradientLeft =
                ...
                Eigen::Matrix<real, 3, 5> flowGradientRight = flowGradientLeft;
                ...
                if (faceToCell[1] != UnInitIndex)
                {
                    ...
                    TransformSpatialGradient<gDim>(_mesh, 1, _mesh->GetFaceZone(iFace), flowSpatialTranspose);
                    flowGradientRight.template topRows<gDim>() =
                    ...
                    TransformSpatialGradient<gDim>(_mesh, 1, _mesh->GetFaceZone(iFace), turbulenceSpatialTranspose);
                ...
                else
                {
                    ...
                    if (_boundaryHandler->GetTypeFromID(
                            _mesh->GetFaceZone(iFace)) == BoundaryType::BCFar &&
                        Velocity(leftFlow).dot(normal) >= 0)
                    {
                ...
                const Eigen::Matrix<real, 3, 5> faceFlowGradient =
                    CorrectedFaceGradient(
                        PrimitiveGradient(leftFlow, flowGradientLeft),
                        PrimitiveGradient(rightFlow, flowGradientRight),
                        PrimitiveState(State(flow[faceToCell[0]])),
                        PrimitiveState(rightCellFlow),
                        centerDisplacement,
                        normal);
                ...
                [&](TurbulenceState &contribution, int iG, real weight)
                {
                    ...
                    if (faceToCell[1] != UnInitIndex)
                    {
                        ...
                        Eigen::Matrix<real, nStoredVariables, gDim> spatialTranspose =
                            gradientRight.template topRows<gDim>().transpose();
                        TransformSpatialGradient<gDim>(_mesh, 1, _mesh->GetFaceZone(iFace), spatialTranspose);
                        gradientRight.template topRows<gDim>() =
                    ...
                    const TurbulenceState faceTurbulence = ClampTurbulenceState(
                        0.5 * (leftTurbulence + rightTurbulence),
                        _turbulenceSettings);
                    DNDS_check_throw_info(static_cast<bool>(_massFlux), "RANS requires shared flow mass flux");
                    const real massFlux = _massFlux(iFace, iG);
                    TurbulenceState advectiveFlux =
                        massFlux * (massFlux >= 0 ? leftTurbulence : rightTurbulence);
                    if (faceToCell[1] == UnInitIndex)
                    {
                    ...
                    const TurbulenceState diffusiveFlux =
                        0.5 * (leftFlow(0) + rightFlow(0)) * TurbulenceDiffusiveFlux(
                            faceTurbulence,
                            faceGradient,
                            normal,
                            _faceWallDistance[static_cast<std::size_t>(iFace)],
...
#    pragma omp parallel for schedule(runtime)
        ...
        for (index iCell = 0; iCell < _mesh->NumCell(); iCell++)
        {
            const TurbulenceState source = TurbulenceSource(
                ...
                _flowSettings.dynamicViscosity,
                _turbulenceSettings);
            rhs[iCell] += flow[iCell](0) * source;
            if (_turbulenceSettings.model == TurbulenceModel::SpalartAllmaras)
            {
                Vector3 densityGradient = Vector3::Zero();
                densityGradient.template head<gDim>() = _flowGradient[iCell].col(0);
                const Vector3 saGradient = CellTurbulenceGradient(iCell).col(0);
                rhs[iCell](0) += SADensityDiffusionCorrection(
                    flow[iCell](0), _flowSettings.dynamicViscosity, _state[iCell](0),
                    densityGradient, saGradient);
            }
            DNDS_check_throw_info(
                rhs[iCell].allFinite(),
...
#    pragma omp parallel for schedule(runtime)
            ...
            for (index iCell = 0; iCell < _mesh->NumCell(); iCell++)
            {
                ...
                const real dt = boundedStageTimeStep(
                    state0[ii], TurbulenceState(_rhs[iCell]) / flow[iCell](0), requestedDt);
                state1[ii] = ClampTurbulenceState(
                    state0[ii] + dt * TurbulenceState(_rhs[iCell]) / flow[iCell](0),
                    _turbulenceSettings);
...
#    pragma omp parallel for schedule(runtime)
            ...
            for (index iCell = 0; iCell < _mesh->NumCell(); iCell++)
            {
                ...
                const real requestedDt = flowPseudoTimeStep[ii] *
                                         ...
                                         static_cast<real>(_turbulenceSettings.transportSubsteps);
                const real dt = boundedStageTimeStep(
                    state1[ii], TurbulenceState(_rhs[iCell]) / flow[iCell](0), requestedDt);
                state2[ii] = ClampTurbulenceState(
                    0.75 * state0[ii] +
                        0.25 * (state1[ii] + dt * TurbulenceState(_rhs[iCell]) / flow[iCell](0)),
                    _turbulenceSettings);
...
#    pragma omp parallel for schedule(runtime)
            ...
            for (index iCell = 0; iCell < _mesh->NumCell(); iCell++)
            {
                ...
                const real requestedDt = flowPseudoTimeStep[ii] *
                                         ...
                                         static_cast<real>(_turbulenceSettings.transportSubsteps);
                const real dt = boundedStageTimeStep(
                    state2[ii], TurbulenceState(_rhs[iCell]) / flow[iCell](0), requestedDt);
                _state[iCell] = ClampTurbulenceState(
                    (1.0 / 3.0) * state0[ii] +
                        (2.0 / 3.0) *
                            (state2[ii] + dt * TurbulenceState(_rhs[iCell]) / flow[iCell](0)),
                    _turbulenceSettings);
            }
        }
    ...
    template <int gDim>
    /** @copydoc ACMTurbulenceTransport::CommitPhysicalHistory */
    void ACMTurbulenceTransport<gDim>::CommitPhysicalHistory(const TFlowDof &flow, bool initialize)
    {
        if (!initialize) _previousPrevious = _previous;
        _previous.resize(_mesh->NumCell());
        for (index i = 0; i < _mesh->NumCell(); ++i)
            _previous[i] = flow[i](0) * TurbulenceState(_state[i]);
        if (initialize) _previousPrevious = _previous;
    }

    template <int gDim>
    /** @copydoc ACMTurbulenceTransport::PhysicalDefect */
    real ACMTurbulenceTransport<gDim>::PhysicalDefect(
        TFlowDof &flow, const BDF2Coefficients &coefficients,
        real dt, const ScalarField *pseudoSteps, real time)
    {
        DNDS_check_throw_info(_previous.size() == static_cast<std::size_t>(_mesh->NumCell()),
                              "Initialize conservative RANS history before BDF");
        EvaluateRHS(_rhs, flow, time);
        for (index i = 0; i < _mesh->NumCell(); ++i)
        {
            const real rho = flow[i](0);
            _rhs[i] -= (coefficients.current * rho * TurbulenceState(_state[i]) +
                        coefficients.previous * _previous[i] +
                        coefficients.previousPrevious * _previousPrevious[i]) / dt;
        }
        const real norm = _rhs.norm2() /
            std::sqrt(std::max(real(1), real(_mesh->NumCellGlobal() * ActiveVariableCount())));
        if (pseudoSteps)
            for (index i = 0; i < _mesh->NumCell(); ++i)
            {
                const real rho = flow[i](0);
                const real denominator = rho / ((*pseudoSteps)[i] * _turbulenceSettings.transportTimeScale) +
                                         coefficients.current * rho / dt;
                const TurbulenceState oldState = _state[i];
                const TurbulenceState increment = TurbulenceState(_rhs[i]) / denominator;
                real relaxation = 1;
                for (int k = 0; k < ActiveVariableCount(); ++k)
                {
                    if (increment(k) < 0)
                        relaxation = std::min(relaxation,
                            0.9 * (oldState(k) - _turbulenceSettings.minimumValue[k]) / (-increment(k)));
                    else if (increment(k) > 0)
                        relaxation = std::min(relaxation,
                            0.9 * (_turbulenceSettings.maximumValue[k] - oldState(k)) / increment(k));
                }
                _state[i] = oldState + std::clamp(relaxation, real(0), real(1)) * increment;
            }
        return norm;
    }
}

...
namespace DNDS::ACMVariable
{
    ...
    {
        /**
         * @brief Reject an inadmissible stage collectively before a halo exchange.
         * @param states Owned state values on this rank; empty ranks are permitted.
         * @param floor Strict positive density threshold from the case configuration.
         * @param mpi Optional initialized communicator for rank-consistent rejection.
         * @author Runzhi Ma
         */
        void ValidateStage(const StateField &states, real floor, const MPIInfo *mpi)
        {
            int valid = 1;
            for (const State &state : states)
                if (!state.allFinite() || state(0) <= floor)
                    valid = 0;
            int globalValid = valid;
            if (mpi != nullptr)
            {
                int initialized = 0;
                MPI_Initialized(&initialized);
                if (initialized)
                    MPI_Allreduce(&valid, &globalValid, 1, MPI_INT, MPI_MIN, mpi->comm);
            }
            DNDS_check_throw_info(globalValid != 0,
                "ACMVariable stage rejected collectively: invalid density/state; reduce CFL or dTau");
        }
...
#    pragma omp parallel for reduction(+ : localSquaredNorm) schedule(runtime)
            ...
            unsigned long long localComponents = static_cast<unsigned long long>(field.size()) * 5ULL;
...
#    pragma omp parallel for schedule(runtime)
    ...
    {
        // Fail before mesh construction, never silently substitute an Euler time model.
        // Author: Runzhi Ma. Physical ACM is a DAE; pseudo-time ACM is an ODE.
        const auto supported = [](int code)
        {
            return code == -1 || code == 0 || code == 1 || code == 2 ||
                   code == 101 || code == 102 || code == 103 ||
                   (code >= 202 && code <= 204) || code == 401 ||
                   (code >= 411 && code <= 413);
        };
        DNDS_check_throw_info(supported(physicalODECode) && supported(pseudoODECode),
                             "Unknown ACMVariable shared ODE code");
        DNDS_check_throw_info(physicalODECode < 0 || pseudoODECode < 0,
                             "Select physicalODECode OR pseudoODECode, not both");
        DNDS_check_throw_info(physicalODECode != 2 && physicalODECode != 101,
            "Physical ACM code 2/101 needs an endpoint projection; use pseudoODECode or a stiffly accurate physical scheme");
...
#    pragma omp parallel for schedule(runtime)
        ...
        const MPIInfo *mpi)
    {
        ...
        ValidateStage(states, settings.densityFloor, mpi);
...
#    pragma omp parallel for schedule(runtime)
            ...
            for (index i = 0; i < static_cast<index>(states.size()); i++)
            {
                ...
                const Matrix5 diagonal =
                    PseudoTimeProductJacobian(states[ii], statesOld[ii], pseudoTimeStep[ii], settings) -
                    diagonalJacobian[ii];
                const Eigen::FullPivLU<Matrix5> factorization(diagonal);
                ...
                real relaxation = timeSettings.implicitRelaxation;
                if (correction(0) < 0)
                    relaxation = std::min(relaxation,
                        0.9 * (states[ii](0) - settings.densityFloor) / (-correction(0)));
                states[ii] += relaxation * correction;

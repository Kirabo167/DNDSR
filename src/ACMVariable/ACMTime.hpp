...
namespace DNDS::ACMVariable
{
    ...
    struct TimeMarchSettings
    {
        ...
        int physicalODECode = -1; ///< -1 native marcher; shared Euler ODE codes: 0,1,102,103,202-204,401,411-413.
        int pseudoODECode = -1; ///< Shared ODE applied to Gamma^-1 R; also supports 101 and explicit code 2.
        ...
        {
            DNDS_FIELD(physicalODECode, "Shared implicit physical ODE code; -1 uses native integrator");
            DNDS_FIELD(pseudoODECode, "Shared pseudo-time ODE code; -1 disables; exclusive with physicalODECode");
            DNDS_FIELD(
                integrator,
    ...
    using MatrixField = std::vector<Matrix5>; ///< Rank-local block-diagonal matrices.

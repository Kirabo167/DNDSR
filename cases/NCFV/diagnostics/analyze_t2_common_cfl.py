"""Audit completed t=2 NCFV runs using recovered density AND pressure point values.
...
from analyze_vortex_convergence import audit_run, exact_density, norms
...
def main():
    ...
    for n in (10, 20, 40, 80):
        ...
        if not (cfg["time"]["cfl"] == 0.5 and cfg["time"]["useCFLTimeStep"] and
                ...
                cfg["physics"]["gamma"] == 1.4 and cfg["initialField"]["vortexStrength"] == 5):
        ...
        if any(audit[label]["gauss_points_stored"] != 0 for label in ("initial", "final")):
            raise ValueError("Efficient solver stored Gauss points")
    ...
    (args.output / "metrics.json").write_text(json.dumps({"common_settings": common_cfg, "runs": runs}, indent=2) + "\n")
    ...
    fig.suptitle("NCFV / thesis normalization / efficient / CFL=0.5 / t=2")
    ...
    report = f"""# NCFV 四套网格：统一 CFL=0.5，推进到 t=2 的精度验证
